...
// --------- Auth Routes ---------
app.post('/api/auth/register', (0, express_validator_1.body)('email').isEmail(), (0, express_validator_1.body)('username').isLength({ min: 3 }), (0, express_validator_1.body)('password').isLength({ min: 8 }), asyncHandler(async (req, res) => {
...
app.post('/api/auth/login', (0, express_validator_1.body)('email').isEmail(), (0, express_validator_1.body)('password').isString(), asyncHandler(async (req, res) => {
...
// --------- Stream Management ---------
app.post('/api/streams/start', jwtAuth, (0, express_validator_1.body)('title').isString(), asyncHandler(async (req, res) => {
...
app.post('/api/streams/stop', jwtAuth, (0, express_validator_1.body)('streamId').isString(), asyncHandler(async (req, res) => {
...
app.post('/api/chat/:streamId', (0, express_validator_1.body)('message').isString(), (0, express_validator_1.body)('username').isString(), asyncHandler(async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    ...
    const chat = await prisma.chatMessage.create({
        data: {
            userId: null, // anonymous or later resolved
            streamId,
            platform,
            username,
            message: cleaned,
...
app.post('/api/webhooks/stripe', express_1.default.raw({ type: 'application/json' }), asyncHandler(async (req, res) => {
...
app.get('/api/oauth/:platform/callback', asyncHandler(async (req, res) => {
    ...
    await prisma.integration.upsert({ where: { userId_platform: { userId: String(userId), platform } }, update: { accessToken: token, connected: true }, create: { userId: String(userId), platform, accessToken: token } });
...
setInterval(async () => {
    try {
        for (const streamId of Object.keys(viewersByStream)) {
            ...
            // update
            await prisma.streamAnalytics.update({ where: { streamId }, data: { peakViewers: Math.max(analytics.peakViewers, viewers), avgViewers: Math.round((analytics.avgViewers + viewers) / 2), totalMessages: newTotalMessages, hourlyStats: hourly } });
            // broadcast stats
